...
def create_indexers_for_corpus(train_paths, indexer_dir, word2vec_model, emb_dim, return_freq_cut_set, wiki2vec_model):
    ere_indexer = Indexer()
    ...
    print("Geo: Before union:", len(token_set))
    token_set = token_set.union(geo_set)
    print("Geo: After union:", len(token_set))
    
    df_person = pd.read_csv("/home/cc/aida-utexas/augment/people_filtered.csv")
    people_set = set(df_person['name'])
    print("Person: Before union:", len(token_set))
    token_set = token_set.union(people_set)
    print("Person: After union:", len(token_set))
    ...
    # Load corresponding pretrained Word2Vec/Wikipedia2vec embeddings
...
def index(data_dir, pre_word2vec_bin, emb_dim, return_freq_cut_set):
    indexed_data_dir = data_dir + '_Indexed_GeoPerson'
    ...
    # Load pretrained Wikipedia2Vec model
    wiki2vec_model = Wikipedia2Vec.load("/home/cc/wikipedia2vec/enwiki_20180420_300d.pkl")

    # Create the indexer
    indexer_info = create_indexers_for_corpus(train_path, indexed_data_dir, word2vec_model, emb_dim, return_freq_cut_set, wiki2vec_model)
    # breakpoint()
