...
def main():
    ...
    ### jy
    # read in csv files of relationship between different claims
    ...
    for query in query_relevant_doc_claim.keys():
        ...
        for relevant_claim in query_relevant_doc_claim[query]:
            json_file_name = relevant_claim_ttl[relevant_claim]
            json_path = os.path.join(args.graph_path, json_file_name +  ".json")
            ...
            kb_path = find_ttl_file(args.kb_path, relevant_claim_ttl[relevant_claim])
            if kb_path is None:
                print("Error: KB not found", relevant_claim_ttl[relevant_claim])
                sys.exit(1)
            ...
            kb_graph.parse(str(kb_path), format='ttl')
