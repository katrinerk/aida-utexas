        ...
        if any([need_atk_set, need_trg_set, need_kle_set, need_vct_set]):
            if need_atk_set or need_trg_set:
                ...
                if cand_stmt.head_id in need_kle_set and '_Killer' in cand_stmt.raw_label:
                    return index
                elif cand_stmt.head_id in need_vct_set and all(
                        [x in cand_stmt.raw_label for x in ['Life.Die', '_Victim']]):
                    if cand_stmt.tail_id not in die_victim_list:
                        return index
        else:
            if (cand_stmt.raw_label, cand_stmt.head_id, cand_stmt.tail_id) in query_stmt_tups:
                continue
            if all([x in cand_stmt.raw_label for x in
                    ['Life.Die', '_Victim']]) and cand_stmt.tail_id in die_victim_list:
                continue
