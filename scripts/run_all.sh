...
print_usage() {
    ...
    printf "[--num_hyps NUM_HYPS] [--num_hops NUM_HOPS] [--coref_compress]\n"
...
./scripts/preprocessing.sh "$input_run_name" "$input_kb_path" "$working_dir" "$num_hyps" "$num_hops" "$do_coref_compression"

echo
echo
echo Start neural.sh ...

./scripts/neural.sh "$working_dir" "$device"

echo
echo
echo Start postprocessing.sh ...

./scripts/postprocessing.sh "$input_run_name" "$run_name" "$output_dir" "$working_dir" "$num_hyps" "$do_coref_compression" "$sin_id_prefix"
