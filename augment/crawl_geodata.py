...
#TODO: 'indonesian' will got a region result. How to bypass it?
    ...
    with open('crawled_geolocation_clean.csv', 'w') as csv_output_file:
        with open('crawled_geolocation_nodata.csv', 'w') as csv_output_file_2:
            ...
            for row in csv_reader:
                ...
                if(geoloc[0].islower()):
                    continue
                g = geocoder.geonames(geoloc, key=user_key[user_idx], maxRows=1)
                # if g.json has return value but g.ok is false, then there must be error msg in json
                while(g.error):
                    # try another user
                    user_idx += 1
                    if(user_idx == len(user_key)):
                        user_idx = 0
                    print("try next user: ",user_key[user_idx])
                    g = geocoder.geonames(geoloc, key=user_key[user_idx], maxRows=1)
                    if(not g.error): 
                       break
                    # else sleep 1 hr   
                    print("Sleep 0.5 hour due to the api error:", g.error)
                ...
                for cand in g:
                    ...
                    g = geocoder.geonames(geoid, method='details', key=user_key[user_idx])
                    ...
                    if g.address == geoloc:
                        ...
                        # e.g. 'Texas' will not return a fully same result.  
