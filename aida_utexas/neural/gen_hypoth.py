...
def evaluate(seed_dir, indexed_data_dir, output_dir, model, device):
    for seed_path in sorted(seed_dir.glob('*_seeds.json')):
        ...
        for idx, indexed_data_path in enumerate(sorted(indexed_data_dir_for_sin.glob('*.p'), key=lambda p: int(p.stem.split('_')[2]))):
            ...
            cluster_seed_stmt_ids = set(seed_json['support'][idx]['statements'])
            ...
            for ere_id in query_ere_ids:
                if graph_mix.eres[ere_id].category == 'Event':
                    killer_set = {graph_mix.stmts[stmt_id].tail_id for stmt_id in set.intersection(graph_mix.eres[ere_id].stmt_ids, cluster_seed_stmt_ids) if '_Killer' in graph_mix.stmts[stmt_id].raw_label}
                    victim_set = {graph_mix.stmts[stmt_id].tail_id for stmt_id in set.intersection(graph_mix.eres[ere_id].stmt_ids, cluster_seed_stmt_ids) if all([x in graph_mix.stmts[stmt_id].raw_label for x in ['Life.Die', '_Victim']])}

                    attacker_set = {graph_mix.stmts[stmt_id].tail_id for stmt_id in set.intersection(graph_mix.eres[ere_id].stmt_ids, cluster_seed_stmt_ids) if '_Attacker' in graph_mix.stmts[stmt_id].raw_label}
                    target_set = {graph_mix.stmts[stmt_id].tail_id for stmt_id in set.intersection(graph_mix.eres[ere_id].stmt_ids, cluster_seed_stmt_ids) if '_Target' in graph_mix.stmts[stmt_id].raw_label}
